...
def calculate_total_units(square_footage, num_bulbs, num_lights, num_refrigerators, num_ovens, num_geysers):
    total_units = 0  # Units for square footage
...
def calculate_bill(total_units):
    if total_units <= THRESHOLD:
        total_bill = total_units * TARIFF_RATE_BELOW_THRESHOLD #4rs
    else:
        total_above_thresh=total_units-THRESHOLD
        total_bill = total_units * TARIFF_RATE_BELOW_THRESHOLD +total_above_thresh * TARIFF_RATE_ABOVE_THRESHOLD #7rs
    return total_bill
